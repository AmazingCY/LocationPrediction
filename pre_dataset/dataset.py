...
class PoiDataset(Dataset):
    ...
    '''
      ...
      split,
      usage 
      len(self.poi2id) :{'22847.0': 0, '420315.0': 1, '316637.0': 2, '16516.0': 3, '5535878.0': 4, '15372.0': 5}
      custom_seq_count)
    '''
    ...
    def __init__(self, users, times, coords, locs, sequence_length, batch_size, split, usage, loc_count,
        ...
        for i, (time, coord, loc, label, lbl_time, lbl_coord) in enumerate(
            ...
            assert seq_count > 0, 'fix seq-length and min-checkins in order to have at least one test sequence in a 80/20 split!'  # 条件不成立执行
            seqs = []
            seq_times = []
            seq_coords = []
            seq_lbls = []
...
class AisDataset(Dataset):
    ...
    def __init__(self, ships, times, coords, SOGs, COGs, locs, sequence_length, batch_size, split, usage, loc_count,
                 custom_seq_count):
        self.ships = ships
        self.times = times
        self.coords = coords
        ...
        self.sequences_labels = []
        self.sequences_lbl_times = []
        self.sequences_lbl_SOGs = []
        self.sequences_lbl_COGs = []
        self.sequences_lbl_coords = []
        self.sequences_count = []
        self.Ps = []
        self.Qs = torch.zeros(loc_count, 1)
        ...
        # align labels to locations (shift by the newest one)
        for i, loc in enumerate(locs):
            self.locs[i] = loc[:-1]  # locations before the newest one
            self.labels.append(loc[1:]) # locations after the latest one
            # adapt time, SOG, COG, and coords:
            self.lbl_times.append(self.times[i][1:])
            self.lbl_SOGs.append(self.SOGs[i][1:])
            self.lbl_COGs.append(self.COGs[i][1:])
            self.lbl_coords.append(self.coords[i][1:])
            self.times[i] = self.times[i][:-1]
            self.SOGs[i] = self.SOGs[i][:-1]
            self.COGs[i] = self.COGs[i][:-1]
            self.coords[i] = self.coords[i][:-1]

        # split to training / test phase:
        for i, (time, sog, cog, coord, loc, label, lbl_time, lbl_sog, lbl_cog, lbl_coord) in enumerate(
                zip(self.times, self.SOGs, self.COGs, self.coords, self.locs, self.labels, self.lbl_times,
                    self.lbl_SOGs, self.lbl_COGs, self.lbl_coords)):
            train_thr = int(len(loc) * 0.8)
            if (split == Split.TRAIN):
                self.times[i] = time[:train_thr]
                self.SOGs[i] = sog[:train_thr]
                self.COGs[i] = cog[:train_thr]
                self.coords[i] = coord[:train_thr]
                self.locs[i] = loc[:train_thr]
                self.labels[i] = label[:train_thr]
                self.lbl_times[i] = lbl_time[:train_thr]
                self.lbl_SOGs[i] = lbl_sog[:train_thr]
                self.lbl_COGs[i] = lbl_cog[:train_thr]
                self.lbl_coords[i] = lbl_coord[:train_thr]
            if (split == Split.TEST):
                self.times[i] = time[train_thr:]
                self.SOGs[i] = sog[train_thr:]
        ...
        for i, (time, sog, cog, coord, loc, label, lbl_time, lbl_sog, lbl_cog, lbl_coord) in enumerate(
                zip(self.times, self.SOGs, self.COGs, self.coords, self.locs, self.labels, self.lbl_times,
                    self.lbl_SOGs, self.lbl_COGs, self.lbl_coords)):
            seq_count = len(loc) // sequence_length
            assert seq_count > 0, 'fix seq-length and records in order to have at least one test sequence in a 80/20 split!'  # 条件不成立执行
            seqs = []
            seq_times = []
            seq_SOGs = []
            seq_COGs = []
            ...
            for j in range(seq_count):
                ...
                seqs.append(loc[start:end])
                seq_times.append(time[start:end])
                seq_SOGs.append(sog[start:end])
                seq_COGs.append(cog[start:end])
                seq_coords.append(coord[start:end])
                seq_lbls.append(label[start:end])
                seq_lbl_times.append(lbl_time[start:end])
                seq_lbl_SOGs.append(lbl_sog[start:end])
                seq_lbl_COGs.append(lbl_cog[start:end])
                seq_lbl_coords.append(lbl_coord[start:end])
            self.sequences.append(seqs)
            self.sequences_times.append(seq_times)
            self.sequences_SOGs.append(seq_SOGs)
            self.sequences_COGs.append(seq_COGs)
            self.sequences_coords.append(seq_coords)
            self.sequences_labels.append(seq_lbls)
            self.sequences_lbl_times.append(seq_lbl_times)
            self.sequences_lbl_SOGs.append(seq_lbl_SOGs)
            self.sequences_lbl_COGs.append(seq_lbl_COGs)
            self.sequences_lbl_coords.append(seq_lbl_coords)
            self.sequences_count.append(seq_count)
            self.capacity += seq_count
            self.max_seq_count = max(self.max_seq_count, seq_count)
        ...
        # statistics
        if (self.usage == Usage.MIN_SEQ_LENGTH):
            print(split, 'load', len(ships), 'users with min_seq_count', self.min_seq_count, 'batches:', self.__len__())
        if (self.usage == Usage.MAX_SEQ_LENGTH):
            print(split, 'load', len(ships), 'users with max_seq_count', self.max_seq_count, 'batches:', self.__len__())
        if (self.usage == Usage.CUSTOM):
            print(split, 'load', len(ships), 'users with custom_seq_count', self.custom_seq_count, 'Batches:',
                  self.__len__())
    ...
    def reset(self):
        ...
        # set active users:
        for i in range(self.batch_size):
            self.next_user_idx = (self.next_user_idx + 1) % len(self.ships)
            self.active_users.append(i)
            self.active_user_seq.append(0)
    ...
    def __len__(self):
        ...
        if (self.usage == Usage.MIN_SEQ_LENGTH):
            # min times amount_of_user_batches:
            return self.min_seq_count * (len(self.ships) // self.batch_size)
        if (self.usage == Usage.MAX_SEQ_LENGTH):
            # estimated capacity:
            estimated = self.capacity // self.batch_size
            return max(self.max_seq_count, estimated)
        if (self.usage == Usage.CUSTOM):
            return self.custom_seq_count * (len(self.ships) // self.batch_size)
        raise ValueError()
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            seqs.append(torch.tensor(self.sequences[i_user][j]))
            times.append(torch.tensor(self.sequences_times[i_user][j]))
            SOGs.append(torch.tensor(self.sequences_SOGs[i_user][j]))
            COGs.append(torch.tensor(self.sequences_COGs[i_user][j]))
            coords.append(torch.tensor(self.sequences_coords[i_user][j]))
            lbls.append(torch.tensor(self.sequences_labels[i_user][j]))
            lbl_times.append(torch.tensor(self.sequences_lbl_times[i_user][j]))
            lbl_SOGs.append(torch.tensor(self.sequences_lbl_SOGs[i_user][j]))
            lbl_COGs.append(torch.tensor(self.sequences_lbl_COGs[i_user][j]))
            lbl_coords.append(torch.tensor(self.sequences_lbl_coords[i_user][j]))
            self.active_user_seq[i] += 1

        history_loc = torch.stack(seqs, dim=1)
        history_t = torch.stack(times, dim=1)
        history_SOG = torch.stack(SOGs, dim=1)
        history_COG= torch.stack(SOGs, dim=1)
        history_coords = torch.stack(coords, dim=1)
        target_loc = torch.stack(lbls, dim=1)
        target_t = torch.stack(lbl_times, dim=1)
        target_SOG = torch.stack(lbl_SOGs, dim=1)
        target_COG = torch.stack(lbl_COGs, dim=1)
        target_coords = torch.stack(lbl_coords, dim=1)
        return history_loc, history_t, history_SOG, history_COG,history_coords, \
            target_loc,target_t,target_SOG,target_COG, target_coords, \
               reset_h, torch.tensor(self.active_users)
